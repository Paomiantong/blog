...
function parseIcon(metastring?: string): React.JSX.Element | null {
  const iconRegex = /icon=(?<quote>["'])(?<icon>[^|]*?)\|?(?<color>[^|]+)\1/;

  const icon = metastring?.match(iconRegex)?.groups?.icon ?? "";
  const iconColor = metastring?.match(iconRegex)?.groups?.color ?? null;

  if (!icon) return null;

  return <Icon icon={icon} width="16" style={{ color: iconColor }} />;
}
